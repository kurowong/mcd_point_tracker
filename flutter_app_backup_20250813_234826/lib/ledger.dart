...
class Ledger {
  ...
  void replay() {
    ...
    for (final txn in ordered) {
      _expire(txn.date);
      if (txn.type == TransactionType.earned) {
  ...
  List<ExpiringBucket> expiringSoon({DateTime? start, int months = 3}) {
    start ??= today;
    final results = <ExpiringBucket>[];
    for (var m = 0; m < months; m++) {
      ...
      results.add(ExpiringBucket(monthStart, total));
    }
    return results;
  }
}
...
class ExpiringBucket {
  ExpiringBucket(this.monthStart, this.total);
